...
%
    ...
    mode_N = false;
    mode_F = false;
    vis_X = 'off'; % remove uicontrol from sight. 
    vis_On ='on'; % turn on to make visible at all times
...
%         step = 0.048;                   % line height
        ...
        % turn off processing steps the user can't access
        settings.read_echointegration = false;
        settings.create_ev_files = 1;
        settings.include_pitch = 1;
        settings.include_roll = 1;
        settings.review_ev_files = 1;
        settings.review_ev_images = 1;
        settings.export_import_HAC = false;
        settings.export_sv = 1;
        settings.read_netcfd = false;
        settings.synTS = true;
        settings.CARS = true;
        settings.npp = true;
        settings.layer_indices = true;
        settings.sound_speed = true;
        settings.read_meta = true;
    ...
    elseif strncmpi(mode, 'NetCDF', 1)  % NetCDF mode
        step = 0.11;                   % line height
        step_EF = 0;
        step_NF = step;
        hght = step * 0.97;             % control height        
        mode_N = true;
        vis_NF = 'on';      
        width = min(max(1100, .55*screen(3)),screen(3));
        set(fig, 'OuterPosition', [.5+(screen(3)-width*0.25)/2 screen(4)/8 width .35*screen(4)]);
        
%         width = min(max(1100, .45*screen(3)),screen(3));
%         set(fig, 'OuterPosition', [.5+(screen(3)-width*0.25)/2 screen(4)/8 width .4*screen(4)]);
       
        % turn off processing steps the user can't access
        settings.create_ev_files = false;
        settings.review_ev_files = false;
        settings.review_ev_images = false;
        settings.export_import_HAC = false;
        settings.export_sv = false;
        settings.read_ecs = false;
              
        settings.review_metadata = false;
        settings.layer_indices = true;
        
        
         % do not run these calculations if in final packaging NF mode     
        settings.sound_speed = 0;
        settings.npp = 0;
        settings.CARS = 0;
        settings.synTS = 0;
        settings.layer_indices= 1;
        settings.read_ecs = 0;                                   
    else
    ...
    uicontrol(main_panel, ...
        ...
        'Value',        pltfm, ...
        'Units',        'normalized', ...
        'Position',     [left base - 0.03 0.25 hght ])
    ...
    uicontrol(main_panel, ...
        ...
        'Visible',      vis_EF, ...
        'Units',        'normalized', ...
        'Position',     [left + 0.25 base - 0.03 bwidth hght ]);
    ...
    uicontrol(main_panel, ...
        ...
        'Visible',      vis_EF, ...
        'Units',        'normalized', ...
        'Position',     [left + 0.3 base - 0.03 bwidth hght ]);
    ...
    for chan = 1:length(settings.channels);
        ...
        'User',         frq, ...
        'Units',        'normalized', ...
        'Position',     [0.45 + chan * 0.05 base - 0.03 0.05 hght ]);
    end
    ...
    uicontrol(main_panel, ...
        ...
        'Units',        'normalized', ...
        'Visible',      vis_On, ...
        'Position',     [bpos base - 0.03 bwidth hght ]);
    ...
    uicontrol(main_panel, ...
        ...
        'Visible',      vis_EF, ...
        'Units',        'normalized', ...
        'Position',     [left base + 0.03 0.2 hght ]);
    ...
    uicontrol(main_panel, ...
        ...
        'Visible',      vis_EF, ...
        'Units',        'normalized', ...
        'Position',     [margin + 0.12 base-0.012 prompt hght ]);
...
%         keyboard
    ...
    uicontrol(main_panel, ...
        'Style',        'edit', ...
        'TooltipString', 'Time PC clock is behind UTC', ...
        'Tag',          'time offset', ...
...
% Haris 05 July 2019 making 'popupmenu' as 'text'. Note that the user should
% not have options to select units. Setting to minutes only. There was some
% issues with toggling fields and if that happens wrong values are passed
% to Echoview.
    ...
    uicontrol(main_panel, ...
        ...
        'Visible',      vis_X, ... %%% Haris 08 July 2019 - it is now mandatory and user will be asked anyway while creating EV files.
        'Units',        'normalized', ...
        'Position',     [bpos base bwidth hght ]);
...
%         'Position',     [left base 0.2 hght ]);
    ...
    uicontrol(main_panel,...
        'Style',        'checkbox', ...
        'TooltipString', 'Review the .ev files by viewing echogram images ', ...
        'Tag',          'review_ev_images', ...
        'String',       'Review echogram images ', ...        
        'Value',        settings.review_ev_images, ...
        'FontWeight',   'bold', ...
        'Visible',      vis_EF, ...
        'Units',        'normalized', ...
        'Position',     [left+0.12 base 0.8 hght ]);
...
% Haris 18/11/2019. Now the HAC files are generated while creating the EV
% file. This will allow to evaluate the effects of filters (Note that HAC
% file is connected to the final cleaned variable in Echoview template). In
% the review process the analyst analyst is expected to define lines and
% regions. There is no need to do HAC export and re-import once again
% because lines are regions are correctly applied in the final cleaned
% Echoview variable.
%
% However, if the ECS file is modified (for calibration correction or any
% other reason), it is import to do HAC export and re-import once again.
% With this the changes made to ECS file will be correctly captured in the
% HAC file and final cleaned Echoview variable.

    uicontrol(main_panel, ...
        'Style',        'checkbox', ...
        'TooltipString', 'Export .ev files to HAC format and re-import', ...
        'Tag',          'export_import_HAC', ...
        'String',       'Export to HAC and re-import (do if *.raw, ECS, gps, pitch, and roll files are modified in the EV file)', ...
        'Value',        settings.export_import_HAC, ...
        'FontWeight',   'bold', ...
        'Visible',      vis_EF, ...
        'Units',        'normalized', ...
        'Position',     [left+0.24 base 0.8 hght ]);
    
    % Export HAC
%     base = base - step_EF; 

% keeping Export HAC to side (see above) and nothing related to HAC is
% displayed in simplified GUI. May want to enable (base = base - step_EF)
% if anything related HAC need to be displayed.
...
% %         'Tag',          'export_sv_fast', ...
% %         'String',       'Fast processing [not yet implemented]', ...            
% %         'Value',        settings.export_sv_fast, ...        
% %         'FontWeight',   'bold', ...
% %         'Visible',      vis_EF, ...
% %         'Units',        'normalized', ...
% %         'Position',     [left+.65 base 0.8 hght ]); 
...
%         'Position',     [right base margin hght ]);
    ...
    uicontrol(main_panel, ...
        ...
        'Visible',      vis_NF, ...
        'Units',        'normalized', ...        
        'Position',     [0.5 base+0.01 0.15 hght*0.85 ]);
...
%         set(gps, 'String', fullfile(pathname, [filename ext]));
        ...
        % Haris 08 July 2019 - fixing issues with auto update path, also
        % including pitch and roll path to auto update
        pitch = get_control(parent, 'transit pitch file');
        roll = get_control(parent, 'transit roll file');
        
        set(gps, 'String', fullfile(pathname));
        set(roll, 'String', fullfile(pathname));
        set(pitch, 'String', fullfile(pathname));        
    end
...
%         title({transit_gps_file, [head ' histogram']}, 'Interpreter', 'none')
        title({[head '_histogram']}, 'Interpreter', 'none')
        xlabel('Seconds');
        ylabel('Count');
...
% Haris 05 July 2019 - making message clear that it is an important step
% user should be aware of this

        uicontrol(tof_panel, ...
            'Style',        'text', ...
            'String',       'This is an important step that determines time offset to UTC (from PC clock). This is calculated from gps.csv file. You are not expected to do anything here just press the button.', ...
            'FontWeight',   'bold', ...
            'FontSize',      11, ...
            'Units',        'normalized', ...
            'Position',     [0 0.5 1 0.35]);
%             'Position',     [0 0.5 1 0.2]);
...
% to Echoview.
    ...
    function start_process(button, ~)
        ...
        if setting.create_ev_files
            ...
            % Haris 18 July 2019 - ask user about beamwidth values updated
            % in the ECS? 
            % Update on 22 November 2019. Improving question dialogue. New
            % IMOS template has precondition for SourceCal numbering in
            % the ECS file.
            
%             q = questdlg('Have you updated manufacturer suggested major and minor beamwidth values in the ECS file?','Check ECS file','Yes','Cancel process to update','Yes');
            
            q = questdlg({'A stitch in time saves nine';''
                '(1) Please use the latest available version of IMOS template';''
                '(2) Have you defined manufacturer specified major and minor beamwidth values in the ECS file? Needed for motion correction';''
                '(3) For EK80 and ES80 data, check "AbsorptionCoefficient" is defined in the ECS file. Needed for secondary corrections ';''
                '(4) In the EV file created, please double-check calibration values are correctly applied to the echogram by checking'
                'Variable Properties > Calibration (at least for the first file)';},...
                'Check Template and ECS file','Yes','Cancel process to update','Yes');
            
            if strcmp(q,'Cancel process to update') | isempty(q)
                msgbox('Processing cancelled by user');
                return
            end            
            ...
            % Haris 08 July 2019 - the time offset obtained from the
            % commented part was not updating 'settings.time_offset' as a
            % result the time offset was not pushing to EV files. Also, at
            % the end of program this was getting over written by the
            % default value. The part of code doing this is commented at
            % the end of this script and moved here. Now making this as a
            % mandatory step to do with no exemption. And hide the
            % visibility of 'Check' buttom from the user.
            
            checkTimeOffset(button) % Gordon's function to check UTC offset, note that it is updating parent
            ...
            % Haris correction on 08 July 2019 ends here.
...
%                 '(3) Please specify "Yes" \ "No" (e.g. Yes or No) for "data_processing_motion_correction" in next stage';''
...
% Haris 04 July 2019: Taking away user choice to define metadata, and make
% it as a mandatory step to do.

            cal_date = uigetdate; % graphical calendar to select calibration date
            setting.meta.calibration_date = datestr(cal_date,29);
            today = char(datetime('now','Format','yyyy-MM-dd'));
            
            if isequal(setting.meta.calibration_date,today)
                warning('Today is selected as calibration date. Makes no sense, select actual calibration date.')
                return
            end
            
            qu = questdlg('The transect data is triangle wave error corrected? Applies to Simrad ES60 and ES70 data only.','Missing metadata', 'Yes', 'No', 'Not applicable','Not applicable');
            setting.meta.data_processing_triwave_correction = qu;

            q = questdlg('Have you applied motion correction while processing data? Press "Yes" if pitch and roll csv files are not dummy. Press "No" if these are dummy files i.e. not containing any measurements.','Missing metadata', 'Yes', 'No', 'Yes');
            setting.meta.data_processing_motion_correction = q;
            
            prompt_h = {'Type transit start locality :','Type transit end locality:'};
            dlgtitle = 'Metadata';
            dims = [1 35];
            definput = {'',''};
            answer = inputdlg(prompt_h,dlgtitle,dims,definput);
            
            setting.meta.transit_start_locality = answer{1};
            setting.meta.transit_end_locality = answer{2};

            setting.meta = review_metadata(setting.meta); % give a chance to review metadata 
            set(get_control(parent,'metadata'),'User',setting.meta);

% Haris correction on 04 July 2019 ends here         
        end
...
%             msgbox(message, 'process_BASOOP completed successfully')
            message = sprintf('Program completed in: %.1f minutes = %.1f hours\n', (telapsed)/60, (telapsed)/3600);
    ...
    function setting = read_settings(button)
        ...
        % Haris 08 July 2019 - moved this commented part to 'Confirm
        % dubious settings' section (see above) - please do not enable this
...
% % % % %             60 ^ (3 - get(get_control(parent, 'time offset units'), 'Value'));
        ...
        setting.review_ev_images = get_boolean(parent, 'review_ev_images');
        ...
        setting.detect_fixed = get_boolean(parent, 'fixed_lines');
        ...
        setting.export_motion_correction_factor = get_boolean(parent, 'export_motion_correction_factor'); % Haris 13/06/2019 to add motion correction factor

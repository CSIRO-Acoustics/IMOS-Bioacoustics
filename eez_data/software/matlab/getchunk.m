...
%    fll           Only with special files:
%              0= [default] values only where mapped - not below seafloor
...
% 
% USAGE: [mn,an,sa,rq,dets,x,y] = getchunk(property,deps,region,pth,fname,opt,fll,vart)

function [mn,an,sa,rq,dets,x,y] = getchunk(property,deps,region,pth,fname,opt,fll,vart)

% MODS:  2/7/08  Prevent leaving file open when user has not requested ANY
%                valid depths.
...
tmp = getnc(cdfile,varn,[dep1 y1 x1],[depn y2 x2],-1,-2,2,0,0);
if dims(tmp)==1

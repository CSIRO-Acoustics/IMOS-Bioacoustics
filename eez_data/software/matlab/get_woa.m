% GET_WOA  Extract a chunk from World Ocean Atlas 2005/2001
%
% INPUT
...
%
%   17/5/2010 Jeff Dunn  CMAR       Supercedes get_woa01.m (JRD 14/4/05)
%
% USAGE: [vv,x,y] = get_woa(par,var,typ,deps,rgn,vers);

function [vv,x,y] = get_woa(par,var,typ,deps,rgn,vers)

if nargin<6 || isempty(vers)
   vers = 5;
end

if vers==1
   [pth,slsh] = path_pc_or_nix('murnau/data/levitus/');
elseif vers==5
   [pth,slsh] = path_pc_or_nix('datalib/climatologies/WOA05_nc/');
else
   disp('Do not know that version of WOA')
   return
end
...
if any(~ismember(deps,depth))
   error('GET_WOA:  not one of the WOA (CSLv1) depths available for this parameter')
else

...
%  deps    vector of DD depths (in m) [sorted but not nec. contiguous] These
%          must be CSIRO Standard Level depths - ie will not interpolate
%          between CARS levels!
%   OPTIONAL:
...
%       15/4/10  Corrected a host of errors which, since some misguided mods
%                in the last 2 years, had disabled all seasonal fitting!!
...
if ~isempty(ic) 
  ...
  if isempty(doy)
    ...
    if tdep>0
       ...
       if t2dep>0
	  st(isnan(st)) = 0;
	  tdoy = tcor.*repmat(doy(ic),t2dep,1);
	  vv(1:t2dep,ic) = vv(1:t2dep,ic) + real(st.*exp(tdoy));
       end
    end
  end
end

...
% export to csv file from a resampled variable.
    ...
    % Haris 09 May 2020: Echoview 11 Exec change and we need a condition to
    % check property or value in Exec that no longer works. Rather than
    % adding a condition for each property and corresponding fields, I am
    % making a general checking based on Echoview version. For Echoview
    % versions <= 10 the existing code should work and no change needed.
    % However, for Echoview version >= 11, some property and corresponding
    % fields were changed. These changes were manually identified with
    % feedbacks from Briony and also by comparing properties between
    % Echoview 10 and 11.
    
    version = str2double(EvApp.version(1:2));
    
    if version <= 10 % For Echoview versions 10 and below pre-existing code works
    ...
    if version >= 11 % For Echoview versions 11 and above, assume Properties won't change in future versions
        EvApp.Exec([vname '| Properties DistanceInMeters =|' num2str((distance))]); % New in Echoview 11
        EvApp.Exec([vname '| Properties DistanceMode =|GPSInMeters']); % New in Echoview 11
        EvApp.Exec([vname '| Properties MaximumRange =|' num2str(max_depth)]);  % set max range
        EvApp.Exec([vname '| Properties MinimumRange =| 0']);  % set min range - always zero
        EvApp.Exec([vname '| Properties Mode =| AllPingsInInterval']); % New in Echoview 11
        EvApp.Exec([vname '| Properties Percentile = |50']); % not applicable when statistic is mean but included for completeness

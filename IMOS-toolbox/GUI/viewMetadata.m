...
%       dimData{k}, ['dimensions{' num2str(k) '}'], lower(dims{k}.name), dateFmt, mode);
  ...
  % Creates a uitable which contains the given data. 
    ...
    % We have removed the replicate button in this version of the IMOS
    % toolbox as it does not seem to serve any purpose. Tim and Haris
    % 14/06/2018
...
% % %         );
    % this was in the old IMOS toolbox but was taken out for some reason.
    % We have reinstated in this version as of 14/06/2018
     doneButton = uicontrol(...
       ...
       'Callback', @(~,~) close(gcf), ...
       'Units',   'normalized', ...
       'Position', posUi2(panel, 10, 10, 3:4, 10, 0) ...
     );
...
% % % %     set(repButton, 'Units', 'normalized'); % removed reference to
% replicate button. Tim and Haris. 14/06/2018
...
% % % %     set(repButton, 'Position', posUi2(panel, 10, 10, 1:2, 10, 0)); % removed reference to
% replicate button. Tim and Haris. 14/06/2018
